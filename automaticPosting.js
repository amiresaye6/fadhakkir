...
const reels = require("./tiktok_videos_data.json");
const sendMessage = require("./sendMessage");
const fs = require("fs").promises;
require("dotenv").config();
const axios = require("axios");
const { getTikTokVideoLink } = require("./tiktioVideoDirectUrl");
...
async function uploadReels() {
  ...
  const { videoLink, description, title } = await getTikTokVideoLink(reel.shareableURL);
  const { hashtags } = reel;
  ...
  try {
    ...
    const result = await postReel(videoLink, description || "", title || "", hashtags || []);
